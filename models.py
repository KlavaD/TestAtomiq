...
class FileDelivery(ABC):
    ...
    def __init__(
        self, src_files, dest_path=None, override=False, dry_run=False
    ):
...
class FTPFileDelivery(FileDelivery):
    ...
    def __init__(
        self, src_files, dest_path, ftp_details, override=False, dry_run=False
    ):
    ...
    def deliver_files(self):
        for src_file in self.src_files:
            dest_file = str(Path(self.dest_path) / src_file.name).replace(
                "\\", "/"
            )
            if self.dry_run:
                print(
                    DRY_MODE_MESSAGE.format(
                        src_file=src_file, dest_file=self.ftp.host
                    )
                )
            else:
...
class OwnCloudFileDelivery(FileDelivery):
    ...
    def __init__(
        self, src_files, owncloud_details, override=False, dry_run=False
    ):
    ...
    def deliver_files(self):
        for src_file in self.src_files:
            if self.dry_run:
                print(
                    DRY_MODE_MESSAGE.format(
                        src_file=src_file, dest_file=self.dest_path
                    )
                )
            else:
                try:
