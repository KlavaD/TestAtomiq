...
class FileDelivery(ABC):
    ...
    def __init__(self, src_file, dest_path=None, override=False, dry_run=False):
...
class LocalFileDelivery(FileDelivery):
    ...
    def deliver_file(self):
        dest_file = Path(self.dest_path) / self.src_file.name
        if self.dry_run:
            ...
            if self.override or not dest_file.exists():
                try:
                    shutil.copy2(self.src_file, dest_file)
...
class FTPFileDelivery(FileDelivery):
    ...
    def __init__(self, src_file, dest_path, ftp_details, override=False,
                 dry_run=False):
        super().__init__(src_file, dest_path, override, dry_run)
        self.ftp = FTP(ftp_details['host'])
        self.ftp.login(ftp_details['username'], ftp_details['password'])

    def deliver_file(self):
        dest_file = str(
            Path(self.dest_path) / self.src_file.name
        ).replace("\\", "/")
        ...
        if self.dry_run:
            ...
            if self.override or not (self.src_file.name in self.ftp.nlst()):
...
class OwnCloudFileDelivery(FileDelivery):
    ...
    def __init__(self, src_file, owncloud_details, override=False,
                 dry_run=False):
        super().__init__(src_file, owncloud_details["url"], override, dry_run)
    ...
    def deliver_file(self):
        if self.dry_run:
            ...
            try:
                self.oc.file_info(self.src_file.name)
                file_not_exist = False
            except owncloud.HTTPResponseError:
                file_not_exist = True
            if self.override or file_not_exist:
                self.oc.drop_file(self.src_file)
                print(MESSAGE_SUCCESS_COPIED.format(
                        src_file=self.src_file,
                        dest_file=self.dest_path
                    ))
